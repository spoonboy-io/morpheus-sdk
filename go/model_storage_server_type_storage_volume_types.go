/*
 * Morpheus API
 *
 * Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.
 *
 * API version: 6.2.1
 * Contact: dev@morpheusdata.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// StorageServerTypeStorageVolumeTypes struct for StorageServerTypeStorageVolumeTypes
type StorageServerTypeStorageVolumeTypes struct {
	Id *int64 `json:"id,omitempty"`
	Code *string `json:"code,omitempty"`
	Name *string `json:"name,omitempty"`
	DisplayOrder *int64 `json:"displayOrder,omitempty"`
	DefaultType *bool `json:"defaultType,omitempty"`
	CustomLabel *bool `json:"customLabel,omitempty"`
	CustomSize *bool `json:"customSize,omitempty"`
	CustomSizeOptions NullableString `json:"customSizeOptions,omitempty"`
}

// NewStorageServerTypeStorageVolumeTypes instantiates a new StorageServerTypeStorageVolumeTypes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStorageServerTypeStorageVolumeTypes() *StorageServerTypeStorageVolumeTypes {
	this := StorageServerTypeStorageVolumeTypes{}
	return &this
}

// NewStorageServerTypeStorageVolumeTypesWithDefaults instantiates a new StorageServerTypeStorageVolumeTypes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStorageServerTypeStorageVolumeTypesWithDefaults() *StorageServerTypeStorageVolumeTypes {
	this := StorageServerTypeStorageVolumeTypes{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetId() int64 {
	if o == nil || o.Id == nil {
		var ret int64
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetIdOk() (*int64, bool) {
	if o == nil || o.Id == nil {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasId() bool {
	if o != nil && o.Id != nil {
		return true
	}

	return false
}

// SetId gets a reference to the given int64 and assigns it to the Id field.
func (o *StorageServerTypeStorageVolumeTypes) SetId(v int64) {
	o.Id = &v
}

// GetCode returns the Code field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetCode() string {
	if o == nil || o.Code == nil {
		var ret string
		return ret
	}
	return *o.Code
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetCodeOk() (*string, bool) {
	if o == nil || o.Code == nil {
		return nil, false
	}
	return o.Code, true
}

// HasCode returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasCode() bool {
	if o != nil && o.Code != nil {
		return true
	}

	return false
}

// SetCode gets a reference to the given string and assigns it to the Code field.
func (o *StorageServerTypeStorageVolumeTypes) SetCode(v string) {
	o.Code = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetName() string {
	if o == nil || o.Name == nil {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetNameOk() (*string, bool) {
	if o == nil || o.Name == nil {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasName() bool {
	if o != nil && o.Name != nil {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *StorageServerTypeStorageVolumeTypes) SetName(v string) {
	o.Name = &v
}

// GetDisplayOrder returns the DisplayOrder field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetDisplayOrder() int64 {
	if o == nil || o.DisplayOrder == nil {
		var ret int64
		return ret
	}
	return *o.DisplayOrder
}

// GetDisplayOrderOk returns a tuple with the DisplayOrder field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetDisplayOrderOk() (*int64, bool) {
	if o == nil || o.DisplayOrder == nil {
		return nil, false
	}
	return o.DisplayOrder, true
}

// HasDisplayOrder returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasDisplayOrder() bool {
	if o != nil && o.DisplayOrder != nil {
		return true
	}

	return false
}

// SetDisplayOrder gets a reference to the given int64 and assigns it to the DisplayOrder field.
func (o *StorageServerTypeStorageVolumeTypes) SetDisplayOrder(v int64) {
	o.DisplayOrder = &v
}

// GetDefaultType returns the DefaultType field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetDefaultType() bool {
	if o == nil || o.DefaultType == nil {
		var ret bool
		return ret
	}
	return *o.DefaultType
}

// GetDefaultTypeOk returns a tuple with the DefaultType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetDefaultTypeOk() (*bool, bool) {
	if o == nil || o.DefaultType == nil {
		return nil, false
	}
	return o.DefaultType, true
}

// HasDefaultType returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasDefaultType() bool {
	if o != nil && o.DefaultType != nil {
		return true
	}

	return false
}

// SetDefaultType gets a reference to the given bool and assigns it to the DefaultType field.
func (o *StorageServerTypeStorageVolumeTypes) SetDefaultType(v bool) {
	o.DefaultType = &v
}

// GetCustomLabel returns the CustomLabel field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetCustomLabel() bool {
	if o == nil || o.CustomLabel == nil {
		var ret bool
		return ret
	}
	return *o.CustomLabel
}

// GetCustomLabelOk returns a tuple with the CustomLabel field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetCustomLabelOk() (*bool, bool) {
	if o == nil || o.CustomLabel == nil {
		return nil, false
	}
	return o.CustomLabel, true
}

// HasCustomLabel returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasCustomLabel() bool {
	if o != nil && o.CustomLabel != nil {
		return true
	}

	return false
}

// SetCustomLabel gets a reference to the given bool and assigns it to the CustomLabel field.
func (o *StorageServerTypeStorageVolumeTypes) SetCustomLabel(v bool) {
	o.CustomLabel = &v
}

// GetCustomSize returns the CustomSize field value if set, zero value otherwise.
func (o *StorageServerTypeStorageVolumeTypes) GetCustomSize() bool {
	if o == nil || o.CustomSize == nil {
		var ret bool
		return ret
	}
	return *o.CustomSize
}

// GetCustomSizeOk returns a tuple with the CustomSize field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StorageServerTypeStorageVolumeTypes) GetCustomSizeOk() (*bool, bool) {
	if o == nil || o.CustomSize == nil {
		return nil, false
	}
	return o.CustomSize, true
}

// HasCustomSize returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasCustomSize() bool {
	if o != nil && o.CustomSize != nil {
		return true
	}

	return false
}

// SetCustomSize gets a reference to the given bool and assigns it to the CustomSize field.
func (o *StorageServerTypeStorageVolumeTypes) SetCustomSize(v bool) {
	o.CustomSize = &v
}

// GetCustomSizeOptions returns the CustomSizeOptions field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StorageServerTypeStorageVolumeTypes) GetCustomSizeOptions() string {
	if o == nil || o.CustomSizeOptions.Get() == nil {
		var ret string
		return ret
	}
	return *o.CustomSizeOptions.Get()
}

// GetCustomSizeOptionsOk returns a tuple with the CustomSizeOptions field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StorageServerTypeStorageVolumeTypes) GetCustomSizeOptionsOk() (*string, bool) {
	if o == nil  {
		return nil, false
	}
	return o.CustomSizeOptions.Get(), o.CustomSizeOptions.IsSet()
}

// HasCustomSizeOptions returns a boolean if a field has been set.
func (o *StorageServerTypeStorageVolumeTypes) HasCustomSizeOptions() bool {
	if o != nil && o.CustomSizeOptions.IsSet() {
		return true
	}

	return false
}

// SetCustomSizeOptions gets a reference to the given NullableString and assigns it to the CustomSizeOptions field.
func (o *StorageServerTypeStorageVolumeTypes) SetCustomSizeOptions(v string) {
	o.CustomSizeOptions.Set(&v)
}
// SetCustomSizeOptionsNil sets the value for CustomSizeOptions to be an explicit nil
func (o *StorageServerTypeStorageVolumeTypes) SetCustomSizeOptionsNil() {
	o.CustomSizeOptions.Set(nil)
}

// UnsetCustomSizeOptions ensures that no value is present for CustomSizeOptions, not even an explicit nil
func (o *StorageServerTypeStorageVolumeTypes) UnsetCustomSizeOptions() {
	o.CustomSizeOptions.Unset()
}

func (o StorageServerTypeStorageVolumeTypes) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Id != nil {
		toSerialize["id"] = o.Id
	}
	if o.Code != nil {
		toSerialize["code"] = o.Code
	}
	if o.Name != nil {
		toSerialize["name"] = o.Name
	}
	if o.DisplayOrder != nil {
		toSerialize["displayOrder"] = o.DisplayOrder
	}
	if o.DefaultType != nil {
		toSerialize["defaultType"] = o.DefaultType
	}
	if o.CustomLabel != nil {
		toSerialize["customLabel"] = o.CustomLabel
	}
	if o.CustomSize != nil {
		toSerialize["customSize"] = o.CustomSize
	}
	if o.CustomSizeOptions.IsSet() {
		toSerialize["customSizeOptions"] = o.CustomSizeOptions.Get()
	}
	return json.Marshal(toSerialize)
}

type NullableStorageServerTypeStorageVolumeTypes struct {
	value *StorageServerTypeStorageVolumeTypes
	isSet bool
}

func (v NullableStorageServerTypeStorageVolumeTypes) Get() *StorageServerTypeStorageVolumeTypes {
	return v.value
}

func (v *NullableStorageServerTypeStorageVolumeTypes) Set(val *StorageServerTypeStorageVolumeTypes) {
	v.value = val
	v.isSet = true
}

func (v NullableStorageServerTypeStorageVolumeTypes) IsSet() bool {
	return v.isSet
}

func (v *NullableStorageServerTypeStorageVolumeTypes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStorageServerTypeStorageVolumeTypes(val *StorageServerTypeStorageVolumeTypes) *NullableStorageServerTypeStorageVolumeTypes {
	return &NullableStorageServerTypeStorageVolumeTypes{value: val, isSet: true}
}

func (v NullableStorageServerTypeStorageVolumeTypes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStorageServerTypeStorageVolumeTypes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


