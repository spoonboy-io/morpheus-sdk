...
validate:
	docker run --rm \
	  	-v ${PWD}:/local openapitools/openapi-generator-cli:v3.0.3 validate \
      	-i local/bundled.yaml

get-version:
	docker run --rm \
	  	-v ${PWD}:/local openapitools/openapi-generator-cli:v3.0.3 version
...
go-sdk:
	...
	mkdir go
	docker run --rm \
  		-v ${PWD}:/local openapitools/openapi-generator-cli:v3.0.3 generate \
  		-i local/bundled.yaml \
  		-g go \
  		-o /local/go
