/*
 * Morpheus API
 * Morpheus is a powerful cloud management tool that provides provisioning, monitoring, logging, backups, and application deployment strategies.  This document describes the Morpheus API protocol and the available endpoints. Sections are organized in the same manner as they appear in the Morpheus UI.
 *
 * The version of the OpenAPI document: 6.2.1
 * Contact: dev@morpheusdata.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package org.openapitools.client.model;

import java.util.Objects;
import java.util.Arrays;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;
import org.openapitools.client.model.ClusterUpdatePermissionsResourcePermissionsSites;

/**
 * Map for namespace group and service plan permissions.
 */
@ApiModel(description = "Map for namespace group and service plan permissions.")
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2023-08-21T11:26:47.028Z[GMT]")
public class ClusterNamespaceCreateResourcePermissions {
  public static final String SERIALIZED_NAME_ALL = "all";
  @SerializedName(SERIALIZED_NAME_ALL)
  private Boolean all;

  public static final String SERIALIZED_NAME_SITES = "sites";
  @SerializedName(SERIALIZED_NAME_SITES)
  private List<ClusterUpdatePermissionsResourcePermissionsSites> sites = null;

  public static final String SERIALIZED_NAME_ALL_PLANS = "allPlans";
  @SerializedName(SERIALIZED_NAME_ALL_PLANS)
  private Boolean allPlans;

  public static final String SERIALIZED_NAME_PLANS = "plans";
  @SerializedName(SERIALIZED_NAME_PLANS)
  private List<ClusterUpdatePermissionsResourcePermissionsSites> plans = null;


  public ClusterNamespaceCreateResourcePermissions all(Boolean all) {
    
    this.all = all;
    return this;
  }

   /**
   * Pass true to allow access to all groups
   * @return all
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "Pass true to allow access to all groups")

  public Boolean getAll() {
    return all;
  }


  public void setAll(Boolean all) {
    this.all = all;
  }


  public ClusterNamespaceCreateResourcePermissions sites(List<ClusterUpdatePermissionsResourcePermissionsSites> sites) {
    
    this.sites = sites;
    return this;
  }

  public ClusterNamespaceCreateResourcePermissions addSitesItem(ClusterUpdatePermissionsResourcePermissionsSites sitesItem) {
    if (this.sites == null) {
      this.sites = new ArrayList<ClusterUpdatePermissionsResourcePermissionsSites>();
    }
    this.sites.add(sitesItem);
    return this;
  }

   /**
   * Array of groups that are allowed access
   * @return sites
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "Array of groups that are allowed access")

  public List<ClusterUpdatePermissionsResourcePermissionsSites> getSites() {
    return sites;
  }


  public void setSites(List<ClusterUpdatePermissionsResourcePermissionsSites> sites) {
    this.sites = sites;
  }


  public ClusterNamespaceCreateResourcePermissions allPlans(Boolean allPlans) {
    
    this.allPlans = allPlans;
    return this;
  }

   /**
   * Pass true to allow access to all plans
   * @return allPlans
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "Pass true to allow access to all plans")

  public Boolean getAllPlans() {
    return allPlans;
  }


  public void setAllPlans(Boolean allPlans) {
    this.allPlans = allPlans;
  }


  public ClusterNamespaceCreateResourcePermissions plans(List<ClusterUpdatePermissionsResourcePermissionsSites> plans) {
    
    this.plans = plans;
    return this;
  }

  public ClusterNamespaceCreateResourcePermissions addPlansItem(ClusterUpdatePermissionsResourcePermissionsSites plansItem) {
    if (this.plans == null) {
      this.plans = new ArrayList<ClusterUpdatePermissionsResourcePermissionsSites>();
    }
    this.plans.add(plansItem);
    return this;
  }

   /**
   * Array of plans that are allowed access
   * @return plans
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "Array of plans that are allowed access")

  public List<ClusterUpdatePermissionsResourcePermissionsSites> getPlans() {
    return plans;
  }


  public void setPlans(List<ClusterUpdatePermissionsResourcePermissionsSites> plans) {
    this.plans = plans;
  }


  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    ClusterNamespaceCreateResourcePermissions clusterNamespaceCreateResourcePermissions = (ClusterNamespaceCreateResourcePermissions) o;
    return Objects.equals(this.all, clusterNamespaceCreateResourcePermissions.all) &&
        Objects.equals(this.sites, clusterNamespaceCreateResourcePermissions.sites) &&
        Objects.equals(this.allPlans, clusterNamespaceCreateResourcePermissions.allPlans) &&
        Objects.equals(this.plans, clusterNamespaceCreateResourcePermissions.plans);
  }

  @Override
  public int hashCode() {
    return Objects.hash(all, sites, allPlans, plans);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class ClusterNamespaceCreateResourcePermissions {\n");
    sb.append("    all: ").append(toIndentedString(all)).append("\n");
    sb.append("    sites: ").append(toIndentedString(sites)).append("\n");
    sb.append("    allPlans: ").append(toIndentedString(allPlans)).append("\n");
    sb.append("    plans: ").append(toIndentedString(plans)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

